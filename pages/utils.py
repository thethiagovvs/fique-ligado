...
<style>
  ...
  /* ── Cards ── */
  /* Card padrão: preenche a tela verticalmente usando flex */
  .card {
    background:#fff;
    border-radius:16px;
  ...
  /* Card menor para páginas com logo */
  .card-logo {
    min-height:440px;
  }
  /* Card compacto para 2FA */
  .card-compact {
    min-height:0 !important;
  }
  /* Spacer — empurra o conteúdo restante para baixo dentro do flex */
  .spacer { flex:1; }

  /* ── Tipografia ── */
  .section-title { font-size:16px; font-weight:700; color:#1a237e; margin:14px 0 4px; }
  .body-text      { font-size:14px; color:#333; line-height:1.7; margin:0 0 10px; }
  ...
  .box-blue {
    ...
    font-size:14px; color:#1a237e; line-height:1.65;
  }
  .box-yellow {
  ...
  /* ── Listas ── */
  .clean-list { padding-left:20px; margin:6px 0 0; font-size:14px; line-height:1.85; color:#333; }

  /* ── Stat rows ── */
  .stat-row  { display:flex; gap:10px; align-items:flex-start; }
  .stat-badge {
    background:#1a73e8; color:#fff; font-weight:800; font-size:.95rem;
    border-radius:8px; padding:6px 8px; min-width:50px; text-align:center; flex-shrink:0;
  }
  ...
  div[data-testid="stButton"] > button {
    ...
    height:46px !important; letter-spacing:.5px; transition:background .15s;
  }
  ...
  /* Botão auxiliar do countdown — totalmente oculto */
  ...
  div[data-testid="stTextInput"] input {
    ...
    border-radius:8px !important; border:2px solid #1a73e8 !important;
    color:#1a237e !important; background:#fff !important;
  }
